...
function getTheForkLink(venue) {
  for (const [key, slug] of Object.entries(THEFORK_SLUGS)) {
    if (venue.name.includes(key)) {
      const base = `https://www.thefork.pt/restaurante/${slug}#rankedBy=SEARCH_ENGINE`;
      return THEFORK_AFFILIATE_ID ? `${base}?cc=${THEFORK_AFFILIATE_ID}` : base;
    }
  }
  return null; // Not on TheFork — no button shown
}
...
function MapView({ venues, scores, selectedId, onSelect, buildings, sun }) {
  ...
  useEffect(() => {
    ...
    venues.forEach((v) => {
      ...
      const tfLink = getTheForkLink(v);
      const bookBtn = tfLink ? `<a href="${tfLink}" target="_blank" rel="noopener" style="display:block;text-align:center;font-size:11px;font-weight:600;color:#fff;background:#e8a840;text-decoration:none;padding:7px 12px;border-radius:8px;margin-bottom:4px;">🍴 Book a Table</a>` : "";
    ...
    <button onClick={onClick} className={`w-full text-left transition-all duration-200 rounded-xl border px-3 py-2.5 ${isSelected ? "border-amber-500/40 bg-amber-900/15" : "border-white/[0.04] bg-white/[0.02] hover:bg-white/[0.04] hover:border-white/[0.08]"}`}>
      ...
      {isSelected && (
        <div className="mt-2.5 ml-4">
          ...
          {/* CTAs */}
          {(() => {
            const tfLink = getTheForkLink(venue);
            return (
              <div className="flex gap-2">
                {tfLink && (
                  <a href={tfLink} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="flex-1 flex items-center justify-center gap-1.5 text-xs font-medium px-3 py-2 rounded-lg bg-amber-500/20 text-amber-400 border border-amber-500/30 hover:bg-amber-500/30 transition-colors">
                    🍴 Book a Table
                  </a>
                )}
                <a href={getMapsLink(venue)} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className={`${tfLink ? "flex-1" : "w-full"} flex items-center justify-center gap-1.5 text-xs font-medium px-3 py-2 rounded-lg bg-white/[0.05] text-stone-400 border border-white/[0.08] hover:bg-white/[0.1] transition-colors`}>
                  📍 Directions
                </a>
              </div>
            );
